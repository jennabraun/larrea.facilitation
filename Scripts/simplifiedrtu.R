...
simp.rtu <- filter(simp.rtu, simp != "damaged" & simp != "zeroes")


bees <- filter(simp.rtu, simp == "bee")
bees.ag <- bees %>% group_by(uniID, simp) %>% summarise(Quantity = sum(Quantity)) 
...
bee1 <- glmer.nb(Quantity ~ blooming + treatment + (1|repID), data = bee)
