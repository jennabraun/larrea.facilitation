...
```{r}
c2 <- glmmTMB(total.visits ~ treatment * flowering + flowers.pot + het.annual.floral.density + het.shrub.blooming.neighbours + offset(log(dec.Length)) + (1|repID), family = nbinom2(link = "log"), data = byrep)
summary(c2)
car::Anova(c2, type = 3)
...
c2.i <- glmmTMB(total.visits ~ treatment * flowering + het.annual.floral.density + flowers.pot + offset(log(dec.Length)) + (1|repID), family = nbinom2(link = "log"), data = byrep)
summary(c2.i)
car::Anova(c2.i, type = 3)

c2.f <- glmmTMB(total.flowers ~ treatment * flowering + het.annual.floral.density + flowers.pot + offset(log(dec.Length)) + (1|repID), family = nbinom2(link = "log"), data = byrep)
summary(c2.f)
```
...
c.visits <- glmmTMB(total.visits ~ treatment + flowering + flowers.pot + het.annual.floral.density + het.shrub.blooming.neighbours +  offset(log(dec.Length)) + (1|repID), family = "nbinom2", data = byrep)
summary(c.visits)
car::Anova(c.visits, type = 2)


c.flowers <- glmmTMB(total.flowers ~ treatment + flowering + flowers.pot + het.annual.floral.density + het.shrub.blooming.neighbours +  offset(log(dec.Length)) + (1|repID), family = "nbinom2", data = byrep)
summary(c.flowers)
car::Anova(c.flowers, type = 2)
```
...
mPt <- MASS::glmmPQL(total.visits~ flowering * rtu +  treatment + flowers.pot + het.annual.floral.density + offset(log(dec.Length)), random = ~1|repID, family="quasipoisson", data = byrtu)
summary(mPt)
...
m <- MASS::glmmPQL(total.flowers~flowering * rtu + treatment*flowering + flowers.pot +offset(log(dec.Length)), random = ~1|repID, family="quasipoisson", data = byrtu)
car::Anova(m, type = 3)
...
#proportions should use logistic model

m1 <- glmmTMB(cbind(unique.fl.visited,flower.fov) ~ flowering * flowers.pot + microsite  + (1|repID), family = binomial, data = rtu.data)
...
m2 <- glmmTMB(cbind(unique.fl.visited,flower.fov) ~ flowering + microsite + flowers.pot + (1|repID), family = binomial, data = rtu.data)
summary(m2)
car::Anova(m2, type = 3)
...
m4 <- glmmTMB(Het ~ d.flowers + dN1 + d.S + z +(1|Rep/Flower), family = "nbinom2", data = counts.ag)
summary(m4)
car::Anova(m4, type = 2)

m5 <- glmmTMB(Het ~ d.flowers + d.S + z+ (1|Rep/Flower), family = "nbinom2", data = counts.ag)
summary(m5)


m6 <- glmmTMB(Het ~ d.S + z +(1|Rep/Flower), family = "nbinom2", data = counts.ag)
summary(m6)

null <- glmmTMB(Het ~ z+(1|Rep/Flower), family = "nbinom2", data = counts.ag)
summary(null)
...
ggplot(all.new, aes(x=metric, y=means)) + geom_errorbar(aes(ymin=V5, ymax=V6, fill = bloom), width=.1, position=position_dodge(width=0.5)) + geom_line(aes(fill = bloom),position=position_dodge(width=0.5)) + geom_point(aes(fill = bloom), position=position_dodge(width=0.5), colour="black",pch=21, size=5) + ylim(-1, 1) +  theme_Publication() + theme(axis.text.x=element_text(angle=90, vjust=.5)) + ylab("") + xlab("") + geom_hline(aes(yintercept=0)) + scale_x_discrete(limits=levels(all.new$metric), labels = metrics)+ scale_fill_manual("",values = c("black", "lightgray"), labels = c("Full bloom", "Pre-bloom"))  + theme(legend.position = c(0.8, 0.1)) + coord_flip() + ylab("RII Microsite") + theme(axis.title.x = element_text(size =11)) + guides(fill = guide_legend(nrow = 2, byrow = TRUE)) 
