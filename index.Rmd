...
#additive
m1 <- glmmTMB(total.flowers ~ treatment+mean.Temp + flowering + flowers.pot + het.annual.floral.density +  offset(log(dec.Length)) + (1|repID), family = "nbinom2", data = byrep)
summary(m1)

a1 <-aov(byrep$mean.Temp~byrep$day)
summary(a1)
cor.test(byrep$mean.Temp, byrep$day)
ggplot(byrep, aes(day, mean.Temp)) + geom_smooth()
...
dates <- read.csv("Clean Data/dates.csv")
byrtu <- left_join(byrtu, dates, by = "Date")

sbees <- filter(byrtu, rtu == "syrphid")

m1 <- glmmTMB(total.visits ~ flowering + treatment + flowers.pot + het.annual.floral.density +  offset(log(dec.Length)) + (1|repID), family="poisson", data = sbees)
summary(m1)

m2 <- glmmTMB(total.visits ~ day +flowering + treatment + flowers.pot + het.annual.floral.density +  offset(log(dec.Length)) + (1|repID), family="nbinom2", data = sbees)
summary(m2)

AIC(m1, m2)

m2 <- glmmTMB(total.visits ~ flowering  +  treatment + flowers.pot + het.annual.floral.density + day + offset(log(dec.Length)) + (1|repID), family="nbinom1", data = sbees)

m3 <- glmmTMB(total.visits ~ flowering  +  treatment + flowers.pot + het.annual.floral.density + offset(log(dec.Length)) + (1|repID), ziformula = ~1, family="poisson", data = sbees)
...
testZeroInflation(r1)
testDispersion(r1, alternative = "greater")
